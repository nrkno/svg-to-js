 ...
 * @param {header: Function, parser: Function, filename: String}[]} extensions Optional. List of extensions to generate custom outputs
...
export default function svgToJS (config) {
  ...
  // Merge extensions to result if extensions are specified in config
  [].concat((config.extensions || [])
    .filter(item => item.filename && item.parser))
    .forEach((extension, index) => {
      const customType = `custom_${(index + 1).toString()}`
      config[customType] = extension.filename
      result[customType] = icons.reduce((content, icon) => {
        return content + extension.parser(icon) + '\n'
      }, typeof extension.header === 'function' ? extension.header(banner) + '\n' : '')
    }, {})
