...
export default function svgToJS (config) {
  ...
  for (const file of files) {
    ...
    const svgPresAttrs = parseSVGTagPresentationAttributes(code)
    const svgAttrStr = svgPresAttrs ? Object.keys(svgPresAttrs).reduce((content, key) => content + `${key}="${svgPresAttrs[key]}" `, '').trim() : ''
    const svgAttrStrJsx = svgPresAttrs ? Object.keys(svgPresAttrs).reduce((content, key) => content + `, ${key}: '${svgPresAttrs[key]}'`, '').trim() : ''
  ...
  [].concat((config.customOutputs || [])
    .filter(item => item.parser))
    .forEach((output, index) => {
      const firstLine = output.includeBanner ? `/** ${banner} **/\n` : ''
      const customType = `custom_${(index + 1).toString()}`
      // Only generate file if filename is provided
      if (output.filename) config[customType] = output.filename
      result[customType] = icons.reduce((content, icon) => {
        return content + output.parser(icon) + '\n'
      }, firstLine)
    }, {})
